...
**Vulnerability:** The `apply_config` function in `src/imagine_mcp/relay_setup.py` applied any key-value pair from the untrusted remote relay config payload directly to `os.environ`, creating an environment variable injection risk.
**Learning:** The logic lacked an allowlist validation, assuming the relay payload was benign. This exposes the application if the remote configuration server or OAuth flow is compromised.
**Prevention:** Always validate configuration payloads parsed from untrusted sources against a strict allowlist (e.g., `ALLOWED_CONFIG_KEYS`) before applying them to process environment variables.
