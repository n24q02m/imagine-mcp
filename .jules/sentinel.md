...
## 2024-08-27 - [Environment Variable Injection via Config Forms]
**Vulnerability:** The `apply_config` function in `src/imagine_mcp/relay_setup.py` took arbitrary dictionary keys from the OAuth relay form and injected them directly into `os.environ` without filtering. This could allow an attacker controlling the relay response to inject critical environment variables (like `PATH` or `LD_PRELOAD`) into the host runtime.
**Learning:** Config payloads parsed from untrusted remote endpoints must never map 1:1 into environment variables.
**Prevention:** Implement a strict allowlist (`ALLOWED_CONFIG_KEYS`) for environment variable keys parsed from remote or untrusted config endpoints and silently discard any keys not on the list before assignment.
