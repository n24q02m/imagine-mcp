...
## 2026-08-16 - [HIGH] Environment Variable Injection via Unvalidated Config
**Vulnerability:** The relay setup process blindly applied any user-supplied configuration from the OAuth form to `os.environ` via `apply_config`. This allows attackers to inject arbitrary environment variables (e.g. `LD_PRELOAD`) which could compromise the host process or manipulate application behaviour.
**Learning:** Accepting untrusted input from an external system (like a relay form) and applying it directly to internal system state (like the process environment) is dangerous. Input should always be constrained to known-good values.
**Prevention:** Implement strict allowlisting for environment variables. Ensure that only a verified list of configuration keys (such as `CREDENTIAL_KEYS`, `UNDERSTAND_MODELS`, `GENERATE_MODELS`, and `GENERATE_PROVIDER_PRIORITY`) are processed and applied.
