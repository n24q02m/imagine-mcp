...
## 2024-05-24 - Environment Variable Injection via Untrusted Config Payload
**Vulnerability:** The `apply_config` function in `src/imagine_mcp/relay_setup.py` directly assigned untrusted configuration payload keys to `os.environ` without validation, allowing remote code execution via environment variable injection (e.g. `PYTHONPATH`, `LD_PRELOAD`).
**Learning:** Functions designed to populate `os.environ` from external sources (like OAuth forms or remote relays) must treat the entire dictionary keyset as untrusted, not just the values.
**Prevention:** Strictly validate all configuration keys against a hardcoded allowlist (`ALLOWED_CONFIG_KEYS`) before applying them to `os.environ`.
