...
## 2025-02-05 - Environment Variable Injection via Remote Relay Config
**Vulnerability:** The relay configuration processing in `src/imagine_mcp/relay_setup.py` accepted arbitrary key-value pairs from an untrusted source (`relay_url`) and injected them directly into `os.environ`. This allowed an attacker to overwrite sensitive environment variables (e.g., `LD_PRELOAD`, `PYTHONPATH`) and potentially execute arbitrary code.
**Learning:** This occurred because the `apply_config` function iterated over the provided `config` dictionary without validating the keys against a strict allowlist. It implicitly trusted the structure of the incoming configuration data.
**Prevention:** Always validate configuration keys parsed from external or untrusted sources (e.g., OAuth forms, remote endpoints) against a strict, predefined allowlist (like `ALLOWED_CONFIG_KEYS`) before applying them to process-level state like `os.environ`.
