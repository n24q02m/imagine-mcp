...
## 2026-08-17 - [CRITICAL] Prevent Arbitrary Environment Variable Injection via Relay Setup
**Vulnerability:** The `apply_config` function in `src/imagine_mcp/relay_setup.py` iterated over a configuration dictionary and blindly injected all key-value pairs directly into `os.environ`. This allowed an attacker providing crafted config payloads (via the OAuth browser form or remote relay) to inject arbitrary environment variables, potentially leading to Remote Code Execution (RCE) by manipulating variables like `PYTHONPATH`, `LD_PRELOAD`, or altering expected application states.
**Learning:** Configurations parsed from external sources (even semi-trusted ones like user inputs in a relay setup form) must never be blindly merged into the host environment. The `os.environ` namespace is extremely sensitive, and untrusted writes can trivially lead to full system compromise.
**Prevention:** Always implement a strict allowlist (e.g., `ALLOWED_CONFIG_KEYS`) for configuration payloads before applying them to environment variables or executing sensitive actions. Validate that any incoming keys exist within this set, ignoring or rejecting unallowed inputs.
