...
## 2024-05-24 - Prevent Environment Variable Injection in relay config apply
**Vulnerability:** The `apply_config` function in `src/imagine_mcp/relay_setup.py` took arbitrary dictionary payloads from the remote relay setup and injected them directly into `os.environ`. This allowed an attacker to override arbitrary environment variables, potentially compromising the application's runtime environment or leading to remote code execution.
**Learning:** The vulnerability existed because the payload keys were not strictly validated against an allowlist before being used to mutate the process environment.
**Prevention:** Always validate untrusted configuration payloads against a strict allowlist (e.g., `CREDENTIAL_KEYS`) before applying them to sensitive system state like `os.environ`.
