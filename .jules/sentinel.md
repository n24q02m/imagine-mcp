...
## 2025-02-14 - Prevent Environment Variable Injection in Relay Setup
**Vulnerability:** The relay setup process blindly accepted any key-value pairs submitted from the remote OAuth form, directly injecting them into the server's `os.environ` and persisting them to the per-plugin store. This allowed attackers to overwrite critical application settings, bypass authentication, or potentially execute arbitrary code if sensitive environment variables (e.g. library paths or debugging flags) were manipulated.
**Learning:** This existed because the schema definition (`RELAY_SCHEMA`) was decoupled from the actual payload processing in the backend endpoint. The backend relied on the frontend to send only the correct fields, neglecting server-side validation.
**Prevention:** To avoid this next time, always implement a strict allowlist of accepted keys when parsing configuration payloads from untrusted sources before applying them to `os.environ` or persisting them to storage.
