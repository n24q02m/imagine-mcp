...
## 2026-08-25 - [HIGH] Arbitrary environment variable injection via relay config
**Vulnerability:** The relay configuration parsing loop unconditionally applied all keys provided in the configuration payload directly to `os.environ`. This allowed an attacker in control of the remote relay (or an operator making a mistake) to inject arbitrary, sensitive environment variables like `LD_PRELOAD`.
**Learning:** Configurations sourced from potentially untrusted or unvalidated endpoints must be strictly filtered against a well-defined allowlist before being mapped directly to the application environment, preventing unauthorized side effects at the system or process level.
**Prevention:** Strictly validate incoming configuration keys against an explicitly defined allowlist (`ALLOWED_CONFIG_KEYS`) prior to any assignment to `os.environ`.
