...
## 2024-05-18 - Edge Auth Bypass via URI Obfuscation
**Vulnerability:** The edge authentication gate in `worker.ts` checked `url.pathname === '/mcp' || url.pathname.startsWith('/mcp/')` directly from the Request URL. A malicious user could bypass the authentication check by using URI obfuscation (e.g. `//mcp` or `/%2Fmcp`), allowing unauthorized access to the durable object.
**Learning:** URL objects do not fully decode or normalize all forms of path variation before exposure in `pathname`, necessitating explicit defense-in-depth sanitization at security boundaries.
**Prevention:** Always `decodeURIComponent` and explicitly normalize path strings (e.g., removing redundant slashes via `.replace(/^\/+/, '/')`) before making routing or security assertions. Wrap decoding in `try/catch` to gracefully fall back on malformed URIs.
