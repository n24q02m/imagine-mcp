...
## 2026-08-13 - [CRITICAL] Fix environment variable injection via config
**Vulnerability:** `apply_config` in `src/imagine_mcp/relay_setup.py` unconditionally injected all key-value pairs from untrusted configuration payloads (e.g., remote relays or OAuth forms) into `os.environ`. This allowed arbitrary environment variable injection, potentially compromising the host system or altering application behavior unexpectedly.
**Learning:** Configuration data coming from external or untrusted sources should never be directly mirrored into the process environment without strict validation against an allowlist, as it bypasses the intended configuration surface.
**Prevention:** Always sanitize and strictly validate incoming configuration payloads against an explicit allowlist (like `ALLOWED_CONFIG_KEYS`) before applying them to `os.environ` or using them for critical control flow decisions.
