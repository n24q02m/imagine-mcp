...
## 2026-07-26 - [CRITICAL] Environment variable injection via configuration payloads
**Vulnerability:** The application blindly iterated over external configuration dictionaries and applied every key-value pair to `os.environ` via `apply_config`. This allowed arbitrary environment variable injection if the configuration payload originated from an untrusted source (e.g., remote relays, OAuth forms).
**Learning:** Configurations sourced externally should never be implicitly trusted to update the process environment without strict validation, as injecting arbitrary environment variables can alter process behavior, hijack execution flows, or bypass security constraints.
**Prevention:** Always use an allowlist (like `ALLOWED_CONFIG_KEYS`) to explicitly define which configuration keys are safe and permitted to be applied to the environment. Any key not in the allowlist must be rejected.
