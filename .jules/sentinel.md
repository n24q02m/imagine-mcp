...
## 2026-07-28 - [Path Obfuscation Auth Bypass]
**Vulnerability:** Edge authentication gate in src/worker.ts bypassed using obfuscated URLs (e.g., //mcp or /%2Fmcp) when attempting KV operations through kvOutbound.
**Learning:** Checking strict path strings against unnormalized request paths allows attackers to evade authentication checks before hitting internal endpoints. An unhandled URIError in decodeURIComponent also leads to 500 errors.
**Prevention:** Always decode and normalize URIs (using decodeURIComponent and replace) wrapped in a try/catch block to return 400 Bad Request before conducting path-based security or routing decisions, and correctly handle malformed URIs.
