...
## 2026-07-29 - [CRITICAL] Environment variable injection in relay config
**Vulnerability:** The `apply_config` function in `src/imagine_mcp/relay_setup.py` iterated over all keys in the user-provided config dictionary and set them in `os.environ` if they differed from the current values. This allowed an attacker to inject arbitrary environment variables into the server process.
**Learning:** Functions that apply user-provided configuration directly to the process environment (`os.environ`) must strictly validate the keys against an allowlist to prevent environment variable injection, which can lead to critical security compromises.
**Prevention:** Define an explicit allowlist of authorized configuration keys (e.g., `ALLOWED_CONFIG_KEYS`) and validate incoming keys against this allowlist before setting them in the environment, ignoring any unauthorized keys.
