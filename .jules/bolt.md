...
## 2026-08-07 - Consolidate credential and system status gathering
**Learning:** The `config` tool's `status` handler used three separate `asyncio.to_thread` calls to fetch version, credential state, and configured providers. Since `_creds_state()` internally calls `_providers_configured_live()`, this caused redundant disk reads via `PerPluginStore.load()` and unnecessary context switches.
**Action:** Consolidate related synchronous system state checks into a single helper function (`_get_system_status_sync`) and call it once via `asyncio.to_thread()`, returning a tuple to avoid repeated I/O and thread scheduling overhead.
