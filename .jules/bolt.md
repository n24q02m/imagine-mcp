...
**Action:** When extending test coverage for dispatchers, use a dedicated extension test file to isolate new edge cases and mock all internal network-bound helpers to ensure test suite stability and speed.
## 2026-07-01 - Pipeline URL validation and network requests
**Learning:** In asynchronous operations where multiple distinct awaitable steps (e.g., DNS validation and network fetching) are performed for a list of items, splitting them into separate `asyncio.gather` calls introduces barrier synchronization delays (the second phase cannot start until the slowest item in the first phase completes). Pipelining them into a single async wrapper function and running one `asyncio.gather` allows faster items to proceed immediately.
**Action:** When performing multi-stage async mapping operations over collections, bundle the sequential stages into a single async worker function per item and gather them all at once to eliminate barrier sync delays.
