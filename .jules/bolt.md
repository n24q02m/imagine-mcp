...
## 2026-08-01 - Optimize raw byte usage for base64 fallbacks
**Learning:** In `src/imagine_mcp/providers/grok.py`, when the API falls back to returning a URL instead of a `b64_json` string, the image bytes were being downloaded, encoded to base64, decoded to string, and then immediately decoded back to bytes. This causes redundant CPU overhead on potentially large payload data.
**Action:** When a fallback mechanism fetches binary data (like images) to emulate an API response that usually returns base64, keep the raw bytes and branch the logic instead of passing the raw bytes through an unnecessary encode-decode cycle to fit a downstream single-variable flow.
