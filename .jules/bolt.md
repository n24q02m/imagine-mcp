...
## 2026-06-13 - Optimize FastMCP build_app performance
**Learning:** Defining tools with inline `async def` functions inside the `build_app` factory method recreated those functions every time `build_app` was called. Additionally, as noted in the memory context, it made the `build_app` function overly long and difficult to maintain. Extracting nested tool implementations into separate module-level async functions (e.g., `_tool_understand`) and registering them using `app.tool(...)(_tool_understand)` eliminates redundant function creation overhead on instantiation.
**Action:** Extract nested tool definitions inside `FastMCP` app builders into module-level functions and register them directly to improve performance and code maintainability.
