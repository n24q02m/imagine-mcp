...
## 2024-05-18 - [Optimize File I/O Thread Pool Usage]
**Learning:** Consecutive synchronous file operations (e.g., `mkdir` followed by `write_bytes`) performed sequentially in an async context using `await asyncio.to_thread(...)` introduce significant context-switching and thread-pool scheduling overhead. Furthermore, redundant `exists()` checks before calling `unlink(missing_ok=True)` waste additional thread dispatches.
**Action:** Consolidate multiple synchronous file operations into a single helper function executed via a single `asyncio.to_thread` call to minimize thread scheduling overhead. Additionally, rely on `missing_ok=True` for safe deletions without preemptive `exists()` checks.
