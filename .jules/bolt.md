...
## 2026-08-13 - Consolidate status gathering
**Learning:** Sequential calls to `asyncio.to_thread` for independent synchronous tasks in the same function introduce unnecessary overhead and wait times. Furthermore, nested or redundant calls to functions like `_providers_configured_live` across these separate synchronous branches can trigger duplicate, expensive disk reads (e.g. from `PerPluginStore`).
**Action:** When a function needs to perform multiple related I/O-bound or blocking operations, consolidate them into a single synchronous helper function and execute that helper via a single `asyncio.to_thread()` call to minimize context-switching overhead and eliminate redundant data fetching.
