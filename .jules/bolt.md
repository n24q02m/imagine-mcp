...
## 2024-06-25 - Prevent Redundant I/O from Sequential Thread-Pool Dispatches
**Learning:** Returning a dictionary mapping where values require asynchronous execution via `asyncio.to_thread` for different synchronous functions can lead to redundant disk I/O when those functions query similar data (e.g. `_creds_state` and `_providers_configured_live` reading `PerPluginStore` from disk sequentially).
**Action:** Consolidate multiple sequential read operations into a single synchronous helper and dispatch them once using `asyncio.to_thread(_new_helper_sync)` to minimize thread-pool scheduling overhead and avoid redundant synchronous reads inside an async event loop.
