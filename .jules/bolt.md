...
## 2026-06-13 - Optimize dispatch URL processing pipeline
**Learning:** Functions handling multiple URLs like `_passthrough_understand` and `dispatch_understand` optimized throughput by parallelizing URL validation and network requests into separate sequential `asyncio.gather` calls. However, this introduced a barrier synchronization delay between the steps (all validations must finish before any detection/fetching starts). Pipelining these operations into a single async helper function per URL and executing them via a single `asyncio.gather(*tasks, return_exceptions=True)` eliminates this delay and allows each URL to progress independently.
**Action:** When performing multi-step asynchronous processing on a collection of items, pipeline the steps into a single per-item async function rather than using multiple sequential `gather` barriers.
