...
## 2026-06-13 - Optimize provider deduplication preserving insertion order
**Learning:** In `src/imagine_mcp/server.py`, the functions `_providers_configured` and `_providers_configured_live` used a manual loop and a `seen` set to deduplicate provider names while preserving the order defined by `CREDENTIAL_KEYS`. This is less efficient than using C-level implementations. The `dict.fromkeys()` pattern deduplicates and strictly preserves insertion order without manual iteration and set checks.
**Action:** Use `list(dict.fromkeys(generator_expression))` to efficiently deduplicate items while preserving insertion order, replacing less efficient manual loops and auxiliary `seen` sets.
