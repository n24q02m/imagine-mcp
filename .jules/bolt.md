...
## 2023-10-24 - Pipelining Validation and Network Requests in `asyncio.gather`
**Learning:** Using multiple sequential `asyncio.gather` calls for inherently related operations (e.g., validating a URL, then fetching from it or detecting its media type) introduces unnecessary barrier synchronization overhead. The fastest URL's network request must wait for the slowest URL's validation to complete before beginning.
**Action:** Pipeline validation and the subsequent network request into a single async flow (e.g., `_validate_and_fetch`), and execute them concurrently with a single `asyncio.gather(*tasks, return_exceptions=True)`. This eliminates the barrier synchronization delay and optimizes throughput for mixed-latency tasks.
