...
## 2026-05-28 - Centralize Provider Configuration to Reduce Duplication
**Learning:** Hardcoded provider priorities and duplicate lists of API keys across multiple modules (`dispatcher.py`, `credential_state.py`, `relay_setup.py`) create a maintenance burden and risk of inconsistency. Centralizing this metadata in `config.py` and deriving other lists from it ensures a single source of truth. Moving the priority order to `Settings` also makes it easily configurable without code changes.
**Action:** Centralize cross-component configuration metadata (like provider mappings and priorities) into a single configuration module and derive dependent lists programmatically to ensure consistency and maintainability.
