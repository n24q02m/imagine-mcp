...
## 2026-08-01 - Consolidate iterative synchronous operations in async loops
**Learning:** When executing iterative synchronous operations (like `f.unlink(missing_ok=True)` on a list of temporary files) within an async context, avoid placing `await asyncio.to_thread(...)` inside the loop. Doing so introduces unnecessary thread-pool scheduling and context-switching overhead for each iteration.
**Action:** When performing iterative synchronous operations within an async context, consolidate them by encapsulating the loop inside a single synchronous helper function, and execute that helper via a single `asyncio.to_thread()` call to minimize overhead.
