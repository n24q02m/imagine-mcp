## 2026-05-28 - Optimize default_provider_for lookups
...
## 2026-05-28 - Optimize media type detection concurrency
...
## 2026-05-28 - Optimize redundant network I/O in Gemini multi-URL processing
...
## 2026-05-28 - Optimize environment variable iteration
...
## 2026-05-28 - Parallelize Gemini multimodal URL processing
**Learning:** In `src/imagine_mcp/providers/gemini.py`, `understand_multimodal` processes multiple media URLs sequentially in a loop. Moving this to a `concurrent.futures.ThreadPoolExecutor` mapping reduces latency from O(N) sequential network calls to O(1) parallel bounded calls, providing massive performance improvements for multi-image queries. Thread pooling must carefully manage cleanup lists (`tmp_files`) before starting blocking operations to remain thread-safe on error.
**Action:** Use `ThreadPoolExecutor` and `pool.map` to parallelize multiple network I/O operations and ensure temporary file handles are staged for cleanup before making the potentially blocking or failing network requests.
