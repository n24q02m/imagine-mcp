...
## 2026-08-08 - Consolidate status check to avoid redundant disk reads
**Learning:** The `config` tool's `status` handler issued multiple separate `asyncio.to_thread` calls. Two of those calls internally invoked the same `_providers_configured_live()` function (one directly, one via `_creds_state`), which in turn calls `PerPluginStore.load()`. This resulted in redundant file system reads and unnecessary thread-pool scheduling during a single status check.
**Action:** When gathering multiple pieces of closely related system state that rely on overlapping file I/O operations, encapsulate the gathering into a single synchronous helper function and execute it with a single `asyncio.to_thread` call.
