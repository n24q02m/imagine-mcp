...
## 2024-08-07 - Consolidate synchronous getters in async contexts
**Learning:** In `src/imagine_mcp/server.py`, the `config` tool's `status` handler used multiple `asyncio.to_thread` dispatches for `_get_version`, `_creds_state`, and `_providers_configured_live`. This resulted in redundant thread-pool scheduling overhead and redundant `PerPluginStore` disk reads (since both `_creds_state` and `_providers_configured_live` read the same file).
**Action:** Consolidate multiple synchronous getters into a single synchronous helper function (`_get_system_status_sync`) and execute them via a single `asyncio.to_thread` dispatch. This minimizes thread-pool scheduling and context-switching overhead, and avoids redundant I/O.
