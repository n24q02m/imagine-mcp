...
## 2024-05-14 - Optimize Leaderboard Merging Logic
**Learning:** `merge_ranks` in `scripts/fetch_leaderboards.py` used O(N^2) list comprehensions nested inside a set iteration to merge LBRow ranks. Using dictionary lookups over a combined list directly changes this to an O(N) complexity which reduces execution time from 1.0970s to 0.0019s for arrays of 5000 items.
**Action:** When merging lists based on IDs, consistently use dictionary-based single pass accumulators instead of nested searches inside for loops to achieve significant performance improvements.
