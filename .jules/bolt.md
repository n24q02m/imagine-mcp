...
## 2026-06-13 - Optimize order-preserving list deduplication
**Learning:** In `src/imagine_mcp/server.py`, the functions `_providers_configured` and `_providers_configured_live` were using a manual `for` loop and a `seen` set to deduplicate provider names while preserving insertion order. This pattern introduces overhead from pure Python loop execution. Using `list(dict.fromkeys(...))` with a generator expression offloads the order-preserving deduplication entirely to the native C implementation of dictionaries, achieving a measurable performance improvement and cleaner code.
**Action:** When performing order-preserving deduplication on sequences, prefer `list(dict.fromkeys(generator_expression))` over manual loops with tracking sets to minimize Python-level overhead and leverage native dictionary speed.
