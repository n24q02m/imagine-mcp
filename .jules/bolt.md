...
## 2026-08-21 - Consolidate status disk reads
**Learning:** In the config status endpoint, we were separately dispatching `_creds_state` and `_providers_configured_live` to threads, causing redundant `PerPluginStore` disk reads. Grouping synchronous logic that fetches multiple related pieces of data into a single helper and dispatching it with one `asyncio.to_thread` call reduces thread overhead and redundant disk I/O.
**Action:** When gathering multiple pieces of data that require blocking synchronous reads (e.g. hitting disk storage) in an async endpoint, consolidate them into a single helper function rather than wrapping them individually in `asyncio.to_thread`.
