...
## 2026-08-16 - Consolidate sync status retrieval to reduce disk I/O
**Learning:** In `src/imagine_mcp/server.py`, the `config` tool's `status` handler executed three separate `asyncio.to_thread` dispatches (for `_get_version`, `_creds_state`, and `_providers_configured_live`). Because both `_creds_state` and `_providers_configured_live` invoke `PerPluginStore.load()`, this resulted in redundant disk I/O, AES-GCM decryption, and JSON parsing per status check, alongside unnecessary thread-pool scheduling overhead.
**Action:** Consolidate the data gathering into a single synchronous helper (`_get_system_status_sync`) and dispatch it with a single `asyncio.to_thread` call to eliminate redundant I/O and reduce context-switching.
