...
## 2026-08-14 - Consolidate multiple status thread dispatches
**Learning:** The `config` tool's `"status"` handler previously launched three separate thread dispatches (`asyncio.to_thread`) for `_get_version`, `_creds_state`, and `_providers_configured_live`. The latter two potentially perform redundant `PerPluginStore` disk reads.
**Action:** When gathering multiple distinct pieces of system state that involve synchronous file I/O or other blocking operations in an async handler, consolidate them into a single synchronous helper function and execute it via one `asyncio.to_thread` dispatch. This avoids redundant disk reads and minimizes thread-pool context switching overhead.
