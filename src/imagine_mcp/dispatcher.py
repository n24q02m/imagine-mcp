...
def dispatch_understand(
    ...
    def _process_url(idx: int, url: str) -> str:
        _validate_url(url, f"media_urls[{idx}]")
        return detect_media_type(url)

    # Process URLs concurrently to reduce latency from DNS and HEAD requests
    media_types = list(
        _DISPATCH_POOL.map(
            lambda args: _process_url(args[0], args[1]), enumerate(media_urls)
        )
    )
