...
async def _write_response_async(resp: httpx.Response, dest: Path) -> None:
    max_size = 50 * 1024 * 1024  # 50MB limit to prevent DoS
    bytes_read = 0
    # ⚡ Bolt: Use native async I/O (anyio) to avoid blocking the event loop
    # during file open/close operations. While writes are still offloaded,
    # this standardizes async file handling and prevents event loop starvation.
