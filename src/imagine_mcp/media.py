        ...
        async with client.stream("GET", url, follow_redirects=True, timeout=60) as resp:
            ...
            try:
                async for chunk in resp.aiter_bytes(chunk_size=65536):
                    ...
                    # We must explicitly cast to Any because of a typing issue with asyncio.to_thread and TextIOWrapper/BufferedWriter
                    await asyncio.to_thread(typing.cast(typing.Any, f).write, chunk)
            finally:
                await asyncio.to_thread(typing.cast(typing.Any, f).close)
    except InvalidURLError as e:
        raise httpx.HTTPError(f"Download failed due to invalid redirect: {e}") from e
    except Exception:
