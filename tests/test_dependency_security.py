...
``urllib3`` and ``idna`` carry explicit CVE-driven floors in
``pyproject.toml``. A lock regeneration that silently dropped or weakened
a pin would reintroduce the vulnerability, so the resolved ``uv.lock``
versions are asserted here.
"""
