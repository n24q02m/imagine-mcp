...
def test_render_template_substitutes_env(tmp_path, monkeypatch):
    ...
    tpl.write_text(
        '{"image":"registry.cloudflare.com/${CLOUDFLARE_ACCOUNT_ID}/imagine-mcp:${IMAGE_TAG}",'
    ...
    assert cfg["image"] == "registry.cloudflare.com/acct123/imagine-mcp:v9.9.9"
...
def test_committed_template_renders_to_valid_json(monkeypatch):
    ...
    assert cfg["name"] == "imagine-mcp-worker"
    assert (
